...
class ProductDetail extends PureComponent {
  ...
  routerPushSpecCreate = productID => {
    router.push({
      pathname: '/product/product-spec-create/' + productID,
  ...
  routerProductDetailNewTab = productID => {
    window.open(
      defaultSettings.apiHost + '/api/v1/product_detail_mobile/' + productID + '/',
      '_blank'
    );
  };
  ...
  routerPushSpecEdit = (specID, productID) => {
    router.push({
      pathname: '/product/product-spec-edit/' + specID,
      state: { productID: productID },
    });
  };
      ...
      for (let i = 0; i < specData.length; i++) {
        arr.push(
          <Card
            ...
            extra={
              <span>
                <a onClick={() => this.routerPushSpecEdit(specData[i].id, productID)}>编辑此规格</a>
                <a
                  style={{ marginLeft: 8 }}
                  onClick={() => this.handleModalVisible(true, specData[i].id)}
                >
                  新增砍价商品
                </a>
                <a
          ...
          >
            ...
            <CreateForm
              ...
              productSpecId={specData[i].id}
            />
            <CreateFormGroupon
              {...parentMethodsGroupon}
              modalVisibleGroupon={modalVisibleGroupon}
              productSpecId={specData[i].id}
            />
          </Card>
        );
